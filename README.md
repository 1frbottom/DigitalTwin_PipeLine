# 공개 데이터 기반 현실 교통정보 실시간 동기화 파이프라인

## 1. 프로젝트 개요

### **배경**
현재 디지털 트윈 기술은 시각적 가시화(Unity, Unreal)에 집중되어 있어, 현실 데이터와의 동기화가 미비하거나 실시간성이 부족한 문제가 있습니다. 본 프로젝트는 이 문제를 개선하고자 하는 목적에서 **공공 데이터를 실시간으로 수집, 가공하여 가상 공간(DB)으로 동기화하는 데이터 파이프라인**을 구축했습니다.<br><br>
